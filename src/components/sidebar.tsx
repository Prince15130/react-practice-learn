...
const Sidebar = ({ isOpen, setIsOpen }: SidebarProps) => {
  ...
  return (
    <div
      className={`bg-white text-black transition-all duration-300 ease-in-out text-sm border-2 rounded-md 
        border-[rgba(0,0,0,0.08)] ${isOpen ? "w-64" : "w-16"}`}
    >
      <div className={"p-3 flex items-center justify-between"}>
        {isOpen && (
          <h1
            className={`text-[#3B40E8] font-bold  overflow-hidden transition-all duration-300 ease-in-out text-lg text-nowrap`}
          >
            Dashboard
          </h1>
        )}
        ...
        {navItems.map((item) => (
          <div key={item.title}>
            <div
              className={`${
                isOpen ? "px-3" : "px-5"
              } py-3 hover:bg-[#3B40E8]/10 cursor-pointer flex items-center justify-between`}
              onClick={() =>
                item.hasDropdown &&
                isOpen &&
                setActiveDropdown(
            ...
            >
              <div className={"flex items-center"}>
                ...
                {isOpen && (
                  <span
                    className={
                      "ml-4 whitespace-nowrap overflow-hidden transition-all duration-300 w-32"
                    }
                  >
                    {item.title}
                  </span>
